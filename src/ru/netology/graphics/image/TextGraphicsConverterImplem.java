...
public class TextGraphicsConverterImplem implements TextGraphicsConverter {

    public static double ratioCount = 1;
    private int widthConverter = 300;
    private int heithConverter = 300;
    private double ratioConverter = 4;
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (widthImg > heightImg) {
            double ratioUrl = (double) widthImg / (double) heightImg;
            if (ratioUrl > this.ratioConverter) {
                throw new BadImageSizeException(ratioUrl, this.ratioConverter);
            }
        } else {
            double ratioUrl = (double) heightImg / (double) widthImg;
            if (ratioUrl > this.ratioConverter) {
                throw new BadImageSizeException(ratioUrl, this.ratioConverter);
            }
        }
        ...
        for (double i = 0.5; i <= this.ratioConverter; i+=0.5) {
            if (widthImg / i <= this.widthConverter && heightImg / i <= this.heithConverter) {
                ratioCount = i;
                break;
            }
        }

        int newWidth = (int) (widthImg / ratioCount);
        int newHeight = (int) (heightImg / ratioCount);
        ...
        char [][] listChar = new char[newWidth][newHeight];
        StringBuilder builderChar = new StringBuilder();
        TextColorSchemaImplem schemas = new TextColorSchemaImplem();
        for (int h = 0; h < newHeight; h++) {
            for (int w = 0; w < newWidth; w++) {
                int color = bwRaster.getPixel(w, h, new int[3])[0];
                char c = schemas.convert(color);
                listChar[w][h] = c;
                builderChar.append(listChar[w][h]);
        ...
        return builderChar.toString(); // Возвращаем собранный текст.
    }

    @Override
    public void setMaxWidth(int width) {
        this.widthConverter = width;
    }

    @Override
    public void setMaxHeight(int height) {
        this.heithConverter = height;
    }

    @Override
    public void setMaxRatio(double maxRatio) {
        this.ratioConverter = maxRatio;
    }
